...
var ifJoint = function(site, list_exist, tableName){
    var alllink = allLink(site, tableName);
    var arrLink={};
    var nameTable;
    for(var i = 0; i < alllink.length;i++){
        ...
        if(ft[0]==tableName && !(list_exist.includes(tt[0]))){
            ...
            nameTable=tt[0];
            var a = ifJoint(`${site}`,list_exist, nameTable);
            if (!($.isEmptyObject(a)))
            {
        ...
        if( tt[0] == tableName && !(list_exist.includes(ft[0]))){
            ...
            nameTable=ft[0];
            var b =ifJoint(`${site}`,list_exist, nameTable)
            if (!($.isEmptyObject(b)))
            {
